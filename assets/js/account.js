...
function renderOverview(d){
  ...
  products.innerHTML=d.software.length?d.software.map(p=>`<article class="account-product-card"><div class="account-product-top"><div><span class="account-pill">${esc(p.ownership==="granted"?"Owned":"Purchased")}</span><h3>${esc(p.name)}</h3></div>${p.currentVersion?`<span class="account-version">v${esc(p.currentVersion)}</span>`:""}</div><div class="account-product-meta"><span>${p.purchasedAt?`Purchased ${date(p.purchasedAt)}`:"Included with your account"}</span>${p.amountPaid!==null?`<strong>${money(p.amountPaid)}</strong>`:""}</div>${p.downloadUrl?`<a class="btn btn-primary account-download" href="${esc(p.downloadUrl)}">Download</a>`:`<span class="account-muted-action">Download not published yet</span>`}</article>`).join(""):renderEmpty("No software purchases are attached to this account yet.");
